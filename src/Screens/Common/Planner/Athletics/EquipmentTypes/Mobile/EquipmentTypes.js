...
import { API_GET_Planner_BodyTypes, API_GET_Planner_EquipmentTypes, API_GET_PLANNER_SUBCATEGORIES } from "../../../../../../api/urls";
...
const EquipmentTypes = () => {
  ...
  useEffect(() => {
   ...
   _fetchEquipmentTypesFUNC(route.params.obj.id);
  }, []);
...
const _fetchGolf = async () => {
  navigation.navigate(NavigationStrings.GOLF_STACK);
};
  // Fetching Data from API
  const _fetchEquipmentTypesFUNC = async (id) => {
    const requestBody = {
      loggedUser: {
    ...
    try {
      const response = await axios.post(API_GET_Planner_EquipmentTypes, {
...
<View style={{ flexDirection:'row' ,marginBottom:5}}>
            ...
            <View style={{width:'60%',alignItems:'center'}}>
              ...
              <Text style={{fontSize:20,fontWeight:'bold'}}>EQUIPMENT TYPES</Text></View>
              </View>
        ...
        >
          ...
          >
            <TouchableOpacity
              onPress={() =>
                navigation.navigate(
                  NavigationStrings.ATHELTIC_EXERCISES,
                  { obj: item }
                )
              }
            >
          ...
          <TouchableOpacity
            onPress={() =>
              navigation.navigate(NavigationStrings.ATHELTIC_EXERCISES, {
                obj: item,
              })
            }
          >
